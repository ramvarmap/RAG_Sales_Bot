...
def process_pdf_upload():
    ...
    st.markdown(
        '<h1 class="main-header">📄 PDF Upload & Processing</h1>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<h3 class="sub-header">📤 Upload New Documents</h3>', unsafe_allow_html=True
    )
    ...
    if uploaded_files:
        ...
        for uploaded_file in uploaded_files:
            ...
            try:
                ...
                is_valid, validation_message = validate_document_requirements(
                    uploaded_file
                )
                if not is_valid:
                ...
                st.success(
                    f"✅ Text extracted successfully ({len(pages_data)} pages, {total_text_length} characters)"
                )
    ...
    with col1:
        st.markdown(
            '<h3 class="sub-header">📚 Document History</h3>', unsafe_allow_html=True
        )
    with col2:
        ...
        try:
            ...
            if documents:
                    ...
                    history_data.append(
                        {
                            ...
                            "Status": doc["status"],
                            "Upload Date": (
                                doc["created_date"].strftime("%Y-%m-%d %H:%M:%S")
                                if doc["created_date"]
                                else "Unknown"
                            ),
                        }
                ...
                search_term = st.text_input(
                    "🔍 Search documents:",
                    placeholder="Enter document name to filter...",
                )
                ...
                if search_term:
                    ...
                    st.write(
                        f"📋 Showing {len(filtered_df)} of {len(df)} documents matching '{search_term}'"
                    )
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "Upload Date": st.column_config.TextColumn(
                            "Upload Date", width="medium"
                        ),
                        "File Status": st.column_config.TextColumn(
                            "File Status", width="small"
                        ),
                    },
                )
                ...
                with col2:
                    completed_docs = sum(
                        1 for doc in documents if doc["status"] == "COMPLETED"
                    )
                ...
                if len(documents) > 0:
                    ...
                    for doc in recent_docs:
                        ...
                        with col2:
                            if doc["created_date"]:
                                st.write(
                                    f"📅 {doc['created_date'].strftime('%Y-%m-%d %H:%M')}"
                                )
                            else:
                ...
                st.info(
                    "📭 No documents have been processed yet. Upload your first PDF to get started!"
                )
...
def chat_interface():
    ...
    st.markdown(
        '<h1 class="main-header">💬 RAG Chat Interface</h1>', unsafe_allow_html=True
    )
    ...
    if prompt := st.chat_input("Ask a question about your documents..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("🤖 Thinking..."):
                ...
                query_embeddings = generate_embeddings_for_query(
                    prompt, st.session_state.access_token
                )
                ...
                search_results = search_chunks(
                    st.session_state.connection, query_embeddings, prompt, top_k=15
                )
                ...
                context += (
                    f"3. Include relevant technical specifications when available\n"
                )
                ...
                ai_response = generate_llama_response(
                    prompt, context, st.session_state.access_token
                )
...
def main():
    ...
    page = st.sidebar.selectbox(
        "Choose a page:", ["📄 Upload PDFs", "💬 Chat Interface"]
    )
