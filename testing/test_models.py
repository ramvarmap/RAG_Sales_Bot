...
def test_imports():
    ...
    try:
        from database.operations import create_tables, search_chunks, get_document_history
...
def test_configuration():
    ...
    try:
        ...
        # Test Google config
        required_google_keys = ['project_id', 'location', 'embedding_model', 'credentials_path']
        ...
        required_chunking_keys = ['target_size', 'max_size', 'overlap', 'min_size', 'document_size_limit']
...
def test_google_authentication():
    ...
    try:
        ...
        if not os.path.exists(key_path):
            ...
            print("   Please place your Google service account JSON key at this location")
            return False
...
def test_embedding_generation():
    ...
    try:
        ...
        if embeddings and len(embeddings) > 0:
            print(f"✅ Embedding generation successful (vector size: {len(embeddings)})")
            return True
        else:
    ...
    try:
        ...
        for query, expected in test_queries:
            ...
            if is_valid == expected:
                ...
                print(f"❌ Query validation failed for '{query}': expected {expected}, got {is_valid}")
                return False
